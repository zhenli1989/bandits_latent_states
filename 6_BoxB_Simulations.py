...
list_random_seed = list(range(1951, 1971)) # To reproduce the results, use use 1951 to 2051
    ...
    for batch_len_ in list_batch:
        ...
        for UCB_multiply_ in list_ucb_multipler:
            ...
            for lmd_ in list_lmd:
                ...
                print(lmd_str_)
                if os.path.isfile(
                        f"{PATH_MODELS}/random_seed_{random_seed}/BoxB_B{batch_len_}_C{UCB_multiply_str_}_Lmd{lmd_str_}{prefix_union_bound}.pkl"):
                    print('skip')
                    continue
                    print("------------------------------")
                dict_linear_bandits_ = \
                    {"seed": random_seed, "number_rounds": number_rounds, "lmd": lmd_, "simple_bound":True,
